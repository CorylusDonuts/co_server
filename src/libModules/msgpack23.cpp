...
namespace temp{
	...
	constexpr T byteswap(T value) noexcept {
		...
		if constexpr (sizeof(T) == 1)
			...
			static_assert(sizeof(T) <= 8, "Unsupported integral size");
	}
};
...
namespace msgpack23 {
	...
	private:
		...
		void pack_type(std::pair<First, Second> const &value) {
			pack_type(value.first);
			pack_type(value.second);
		}
	...
	private:
		...
		template<typename First, typename Second>
		void unpack_type(std::pair<First, Second> &value) {
			unpack_type(value.first);
			unpack_type(value.second);
		}
